...
- **Tests are organized by the module under test, not by feature.** There is no `test_<feature>.py` catch-all: tests for `app/utils/*` go in `app/tests/test_utils.py`, tests for `app/templatetags/*` go in `app/tests/test_templatetags.py`, tests for management commands go in `app/tests/test_commands.py`, and tests for views go in `app/tests/views/test_*.py` (e.g. `test_views.py` for misc views, `test_atlas_views.py` for the atlas explorer). When a feature touches several of these layers (e.g. a set of JSON-LD builders, their template tags, a management command, and the pages that render them), its tests get split across the corresponding existing files instead of living together in one new file — see the `bioschemas` feature's tests for a worked example (builders in `test_utils.py`, tag rendering/escaping in `test_templatetags.py`, `dump_bioschemas` in `test_commands.py`, page-level checks in `views/test_views.py::BioschemasViewTest`). Helpers/fixtures used by only one test class belong inside that class, not at module scope.
- Standalone module-level functions used only by a single class (e.g. a `Command` in a management command) are usually better as methods on that class, since they share its context and are easier to find alongside it.
- Recommended, not enforced by lint: avoid one-off helper functions that are called from exactly one place and add no clarity of their own — inline them at the call site instead. Keep comments proportional to the code they annotate; a 2-3 line block of code rarely needs more than a 1-2 line comment explaining the non-obvious "why".
- Don't parse HTML with regex — it's brittle around attribute order, whitespace, and nesting. Use `beautifulsoup4` (`from bs4 import BeautifulSoup`), already a project dependency and used in `app/utils/blog.py`; e.g. `BeautifulSoup(html, "html.parser").find_all("script", type="application/ld+json")` instead of a hand-rolled `<script ...>(.*?)</script>` pattern.
